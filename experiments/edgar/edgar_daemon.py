...
# ---------------------------------------------------------------------------
# Filing retriever — text-first retrieval pipeline
# ---------------------------------------------------------------------------
    ...
    async def retrieve_full(self, discovery: FilingDiscovery) -> bool:
        ...
        **All blocking filesystem (fsync/rename) and SQLite work is offloaded
        ...
        try:
            ...
            # 4. Structured extraction (Form 3/4/5, 8-K)
            form_upper = (header.form_type or "").upper().strip()
            form4: Form4Filing | None = None
            eight_k_events: list[EightKEvent] | None = None

            if _OWNERSHIP_FORM_RE.fullmatch(form_upper) and extracted_bytes is not None:
                try:
                    form4 = parse_form4_xml(extracted_bytes, acc)
                except Exception:
                    logger.exception("Form 4 parse failed for %s (non-fatal)", acc)
            ...
            if form_upper.startswith("8-K"):
                try:
                    ...
                    logger.exception("8-K parse failed for %s (non-fatal)", acc)

            # 5. Commit — offloaded to thread to avoid blocking the event loop
            ...
            if self.commit_service is not None:
                ...
                def _legacy_commit() -> None:
                    ...
                    if form4:
                        self.storage.save_form4_transactions(form4)
                        self.storage.save_form4_holdings(form4)
                    # Always pass accession so stale 8-K rows are cleared
                    # even when the new parse returns zero items.
                    self.storage.save_8k_events(acc, eight_k_events or [])
