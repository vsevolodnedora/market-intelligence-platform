...
no avoidable live rejects; deterministic replay from strategy intent to broker result to journal state, including under broker corrections.
...
At current IBKR Germany public pricing, fixed minimum commissions alone are large relative to a EUR 3,000 account:  
...
A EUR 3 minimum is about **20 bps on a EUR 1,500 ticket** and falls to **10 bps only at EUR 3,000**, before venue fees and spread. 
...
Deutsche Börse publishes the **Xetra Liquidity Measure (XLM)** as a basis-point implicit-transaction-cost measure for specified order sizes, 
and exposes the open order book intended to support trading decisions. These are venue-native liquidity inputs the execution policy must consume (see §13).
...
The system must first prove exact broker reconciliation. It then proves that a low-turnover strategy can economically survive current small-account cost floors. Only after that may it widen the universe or execution scope.
...
The system may paper-trade or simulate other profiles, including SmartRouting fixed, but they are not production-admissible  
until separately validated with observed live broker charges and reconciliation under the fee-closure rule in §12.
...
4. Deterministic replay is defined as: given the archived broker event stream and reference-data snapshots, replay reproduces identical journal state byte-for-byte within the journal's canonical ordering.
...
v1 ships with the current Xetra ETF regime applicable to the production profile. The engine must carry **forward-dated** regime entries so that the EU migration to **T+1 on 11 October 2027** is representable today, with:
...
**No profile becomes production-supported until every fee component has at least one observed live exemplar and the model reproduces posted broker charges within governance tolerance.**  
The required dimensions for exemplar coverage are:

* routing mode (directed vs SmartRouted)
* session regime
* instrument class (UCITS ETF here)
* order lifecycle state (new vs modified vs overnight-persisting)
* retail-vs-direct handling path
...
Every live order must pass a venue-native liquidity check before admission:

* quoted spread at decision time must be **below an ISIN-specific cap** calibrated to the order's notional,
* order must fall within a **pre-approved liquidity window** (not the first or last minutes of continuous trading unless empirical fill quality justifies it),
* the XLM-derived implicit-cost estimate for the order's size must sit within the one-way cost budget used in `N_min`.
...
* cap aggression using the XLM-derived implicit-cost budget,
...
Advance to Tier 1, and then Tier 2, only when the relevant acceptance gates (§17) pass. In practice, advancing past Tier 0 likely requires capital above EUR 3,000 because the fixed minimum commissions are too large for diversified low-ticket rotation. Public IBKR pricing is explicit enough that this constraint is treated as structural.
...
* every fee dimension (route, venue, order class, session, retail-vs-direct, lifecycle state) has at least one observed live exemplar,
* modelled fees reproduce posted broker charges within governance tolerance,
* realised slippage tracks the conservative spread/liquidity model within tolerance,
