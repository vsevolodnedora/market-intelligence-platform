Assume a role of a lead software engineer in quantitative finance and thoroughly and critically analyze the following personal project step-by-step.
...
* monthly evaluation may still be run for research, but live orders are admitted only when score change clears a wide churn band, minimum notional is met, and estimated **all-in** cost remains economically justified after broker commission minimums, third-party venue fees, and expected spread
* this default may be tightened only after live reconciliation shows that smaller tickets remain economical on the chosen route / venue profile

At current IBKR Germany pricing, the commission minima and venue fees are large enough that small-ticket monthly rotation is usually uneconomic: SmartRouting fixed is 0.05% with a EUR 3 minimum, generic direct routing is 0.10% with a EUR 4 minimum, SWB direct is 0.12% with a EUR 6 minimum, and GETTEX direct is 0.05% with a EUR 3 minimum; SWB ETFs also carry a 0.10% exchange fee plus EUR 0.01 regulatory fee, while GETTEX currently shows zero exchange and clearing fees.
...
For IBKR cash accounts, order admissibility is a broker-facing hard constraint. The engine must require enough cash to cover principal, commissions, taxes, and venue fees, and it must require **settled cash** to enter buys. Settlement timing must **not** be hardcoded from a single broker help page: instead, the system maintains an effective-dated **broker / venue / instrument settlement-policy table**, uses that table for order admission and forecasting, and validates it continuously against live broker statements and Flex reports. Any additional broker-side cash buffers for specific order types must be represented as configurable broker rules and must be validated in paper-trade or micro-live before being relied on in simulation or live admission.

IBKR's public docs currently support the need for this wording: one cash-account page says German stock cash becomes available after two days and that cash accounts must have settled cash to enter trades, while another IBKR settled-cash glossary page still uses a generic stock-settlement example of trade date + 3 days.
...
* **document-evidence registry for PRIIPs KID validity, Germany marketability, and tax classification support**
* tax classification
* Teilfreistellung regime
* Basiszins inputs
* corporate-action rules

This fixes the two gaps in the original draft: broker settlement policy was under-modeled, and legal/tax eligibility evidence was not elevated to first-class reference data. The need for dated Basiszins inputs, dated Teilfreistellung handling, and documentary eligibility evidence is supported by InvStG, BMF publication practice, BaFin's own database caveats, and PRIIPs/KID timing.
...
IBKR describes Flex Web Service as the programmatic retrieval path for preconfigured Flex Queries and notes that Activity Statement data is typically updated once daily at close of business.
...
* retained documentary evidence linking each eligibility decision to issuer, exchange, BaFin, and tax-reference sources

Use **issuer KID / prospectus / official fund documents / exchange metadata** as the primary eligibility sources. Use **BaFin's investment-funds database** only as a corroborating control source for Germany marketability, never as the sole authority. BaFin states that the database is published on a voluntary basis, is in principle updated daily, and accepts no liability for completeness or correctness. PRIIPs/KID validation must be date-aware, because the PRIIPs KID regime applies to UCITS from **1 January 2023**.
...
The simulator must enforce the same venue / phase gating, broker settlement policy, fee model, and admissibility logic used in live. A simulation result is invalid if it admits any order that the active cash-account policy, venue-phase policy, or compliance policy would block in live trading.
...
Scheduled **evaluation** may run monthly, but live trading at the initial account size defaults to **threshold-driven rebalancing with quarterly review**. Monthly live rebalance is disabled unless empirical live reconciliation shows that the ticket sizes, routing profile, and all-in costs remain economical.
...
* no simulated order is admitted that would fail live for settled-cash, venue-phase, or compliance reasons
