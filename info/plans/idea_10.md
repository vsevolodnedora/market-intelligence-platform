...
v1 succeeds only if **all four** acceptance gates pass (see §17), **in sequence**. The four criteria are **not co-equal parallel goals**; they are sequential programs, and each gate is admissible only after the prior gates have closed (see §17 "Sequencing rationale"). In summary form:
...
**Per-trade admission.** A rebalance or entry is admitted only if **all** of the following hold at decision time:

1. The **point-estimate** expected excess benefit of the trade versus the do-nothing state exceeds the full modeled cost hurdle:

   ```
   E[ Δα_vs_do_nothing ] > Ĉ_RT + B_model + B_ops
   ```

   where
   * `E[...]` is the point-estimate expected excess benefit under the live strategy's signal,
   * `Ĉ_RT` is expected round-trip all-in cost (commission + venue/clearing/regulatory fees + spread + tax friction; FX = 0 under v1 EUR-only scope),
...
```
LCB[ Δα_vs_do_nothing | strategy, horizon ] > Ĉ_RT + B_model + B_ops
```

computed at the **strategy/decision-category level** over the Gate-3 evaluation horizon, where `LCB[...]` is the pessimistic lower bound (point estimate minus a calibration-derived margin) of strategy-level excess benefit. A strategy whose LCB expires or degrades below hurdle loses live-rotation admission until re-certified; existing positions are not forced liquidated but no new rotation trades are admitted.

This two-level design preserves the LCB discipline where it can be statistically supported (at the strategy level, across many shadow decisions) while keeping per-trade admission decidable with the information actually available at a single trade.
...
* `admission_decision_id` — unique per admission evaluation (links to the per-trade admission-rule inputs used per §6, including the fill-probability estimate from §13 and the strategy-level Gate-3 certification reference)
...
Trades can occur between reviews only if a threshold breach is large enough to clear the per-trade admission hurdle (§6) — i.e. the point-estimate expected excess benefit, taken over the joint fill/no-fill outcome space, exceeds the modelled round-trip cost stack, and the strategy holds a current Gate-3 certification.
...
### Fill-probability and no-fill opportunity cost

For DAY-limit execution, realized implementation shortfall is driven not only by on-fill slippage but also by **queue position, no-fill probability, and the foregone-trade opportunity cost** of an unfilled limit. A deterministic limit-price rule with spread caps addresses on-fill slippage but is silent on whether the order fills at all; without an explicit fill model, shadow-alpha estimates are systematically biased upward because they treat every admitted trade as filled at the chosen limit.

v1 therefore makes fill probability a first-class object in both admission and simulation:

* **Every admission decision records an estimated fill probability** `p_fill` at the chosen limit price, under a documented fill model (below). The point-estimate expected excess benefit in the §6 per-trade admission rule is taken over the **joint** outcome space of fill and no-fill — i.e. `E[Δα] = p_fill · E[Δα | fill] + (1 − p_fill) · E[Δα | no-fill]`, where `E[Δα | no-fill]` is the opportunity cost of remaining in the do-nothing state until the next decision opportunity.
* **The simulator (§15) generates no-fill paths as a modelled outcome**, not as an edge case. Simulator runs track fill-rate, mean time-to-fill, and foregone-alpha on no-fill as first-class output metrics alongside realized slippage on fills.
* **Fill model — bootstrap phase.** Until live exemplars exist to calibrate a per-ISIN fill model, the simulator uses a **conservative bootstrap** fill model: `p_fill` is a monotone-decreasing function of limit aggression inside the spread, anchored to a conservative prior drawn from venue order-book snapshots (queue depth and replenishment rate at the nearest book levels). The bootstrap deliberately under-estimates `p_fill` so that Gate-3 strategy-level LCB is not inflated by optimistic fill assumptions.
* **Fill model — exemplar-calibrated phase.** As live fills and no-fills accumulate under the pinned production profile, a calibrated fill model replaces the bootstrap. The calibration procedure (minimum exemplar counts, calibration window, refresh cadence) is pinned as a governance parameter under §6 before Gate 3 can reference the calibrated model.
* **Re-submit fee interaction.** The per-decision re-submit cap (below) feeds directly into the decision-level fill model: each re-submit adds a fresh minimum-commission charge, and the decision-level admission evaluates `E[Δα]` over the full re-submit-budgeted policy, not just the first limit.

No-fill is treated as a legitimate outcome of the DAY limit: an expired unfilled limit is the cheaper failure mode relative to a modified/overnight order or an over-aggressive resubmit loop. The fill model makes that trade-off explicit rather than assuming it away.
...
* applies the **exact** live admission logic, including the per-trade admission rule (§6), the ticket-size-native liquidity gate (§13), the fill-probability and no-fill model (§13), and the re-submit budget (§13),
...
* the per-trade admission rule (§6), including the joint-outcome `E[Δα]` under the fill-probability model,
...
* the **joint-outcome fill/no-fill model** from §13 — canonical runs produce fill rate, mean time-to-fill, and foregone-alpha on no-fill as first-class output metrics,
...
whose backed-by-historical-research expected excess benefit plausibly exceeds the implied round-trip floor at the intended capital and ticket-size distribution, by a margin that could survive the strategy-level LCB confidence margin.
...
* same admission logic (per-trade rule from §6, ticket-size-native liquidity gate, and fill/no-fill model from §13)
* same fee model
* same journal
...
* the canonical simulator (§15) and the live admission engine share identical admission logic across the per-trade rule (§6), the ticket-size-native liquidity gate (§13), the fill/no-fill model (§13), and the re-submit budget (§13),
...
* realised fill rate at the pinned limit-price construction is within the conservative bootstrap fill model's expected range (§13); systematic deviation triggers a re-calibration before Gate 2 can close,
...
* out-of-sample shadow decisions produce positive strategy-level LCB excess benefit over do-nothing after full modelled costs (`LCB[Δα | strategy, horizon] > Ĉ_RT + B_model + B_ops`),
...
* the **joint-outcome fill/no-fill model** for all limit execution — the point-estimate expected excess benefit used for admission is always taken over fill and no-fill outcomes, not conditional on fill
